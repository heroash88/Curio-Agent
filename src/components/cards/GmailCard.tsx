...
const GmailCard: React.FC<CardComponentProps> = ({
  card,
  onInteractionStart,
  onInteractionEnd,
}) => {
  const t = useCardTheme();
  const d = card.data as unknown as GmailCardData;
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      console.error("[GmailCard] Reply failed:", e);
    } finally {
      setSending(false);
    }
  };
        ...
        {sent ? (
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="rounded-xl px-3 bg-sky-500 text-white hover:bg-sky-600 disabled:opacity-50 shrink-0"
            >
              <Send size={14} />
            </button>
          </div>
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className={`text-base font-bold flex-1 ${t.text}`}>
          {d.mode === "search" ? `Gmail: "${d.query}"` : "Gmail Inbox"}
        </h3>
