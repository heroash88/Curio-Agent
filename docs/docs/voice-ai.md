...
Custom LLM mode supports direct Gemini text models, Ollama, hosted providers, and custom endpoints. The provider list covers Gemini, Ollama, Mistral, Claude, OpenAI, Amazon Nova, Groq, OpenRouter, and Custom (Advanced).
...
- Model name.
- Provider credential when needed. Gemini text uses a dedicated Gemini Text API key, separate from Gemini Live. Hosted/custom provider credentials are encrypted through secret storage and scoped by provider/model/endpoint so switching presets does not reuse another provider's key.
