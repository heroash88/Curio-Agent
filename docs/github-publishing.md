...
The tag push wakes `release.yml`, which:

- Builds the Electron app on `macos-latest`, `windows-latest`, and
  `ubuntu-latest` using the existing `electron:build:{mac,win,linux}`
  scripts.
- macOS produces Intel (x64) and Apple Silicon (arm64) DMGs.
- Windows produces an x64 NSIS installer and an x64 portable exe.
  arm64 is omitted because the combined payload exceeds NSIS's 32-bit
  mmap limit; Windows arm64 users run the x64 installer natively.
- Linux produces an x64 AppImage and an x64 deb. arm64 is omitted
  because the payload exceeds fpm's tar/xz limits on GitHub runners;
  Linux arm64 users use the `rpi-image/` kiosk path.
- Uploads each installer set as a workflow artifact.
- Creates a GitHub Release named `Curio Robot v<version>` and attaches the
  `.dmg`, `.exe`, `.AppImage`, and `.deb` installers to it.
